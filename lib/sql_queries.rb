...
"SELECT projects.title, SUM(pledges.amount) FROM pledges JOIN projects ON pledges.project_id = projects.id GROUP BY projects.title ORDER BY projects.title ASC;"
end
...
"SELECT users.name, SUM(pledges.amount) FROM users JOIN pledges ON pledges.user_id = users.id GROUP BY users.name ORDER BY SUM(pledges.amount), users.name ASC"
end
